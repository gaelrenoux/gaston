...
final class ProblemImpl(
  ...
  lazy val incompatibleTopicsBySlot: BitMap[Slot, Set[Topic]] = {
    val couples = for {
      slot <- slots
      topic <- topics
      if !topic.unassigned // Virtual "unassigned" topics are never moved
  ...
  val preferredTopicsByPerson: BitMap[Person, Seq[Topic]] = preferencesByPerson.map { preferences =>
    preferences.view.collect {
