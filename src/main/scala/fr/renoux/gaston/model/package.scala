...
package object model {
  ...
  implicit final class TopicMapOps[A >: Null](val wrapped: Map[Topic, A]) extends AnyVal {
    @inline def toArrayMap(using counts: Counts, tag: ClassTag[A]): ArrayMap[Topic, A] = toArrayMap()

    @inline def toArrayMap(default: A = null)(using counts: Counts, tag: ClassTag[A]): ArrayMap[Topic, A] =
  ...
  implicit final class PersonMapOps[A >: Null](val wrapped: Map[Person, A]) extends AnyVal {
    @inline def toArrayMap(using counts: Counts, tag: ClassTag[A]): ArrayMap[Person, A] = toArrayMap()

    @inline def toArrayMap(default: A = null)(using counts: Counts, tag: ClassTag[A]): ArrayMap[Person, A] =
