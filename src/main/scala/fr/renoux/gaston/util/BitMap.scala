...
object BitMap {
  def from[A <: Identified, B >: Null : ClassTag](size: Int, default: B = null)(m: Iterable[(A, B)]): BitMap[A, B] = { // scalastyle:ignore null
  ...
  object syntax {
    implicit class BitMapConversionOps[A <: Identified, B >: Null](val wrapped: Iterable[(A, B)]) extends AnyVal {
      ...
      @inline def toBitMap(size: Int)(implicit tagB: ClassTag[B]): BitMap[A, B] =
        BitMap.from[A, B](size, null)(wrapped) // scalastyle:ignore null

      @inline def toBitMap()(implicit tagB: ClassTag[B], count: Count[A]): BitMap[A, B] =
        BitMap.from[A, B](count.value, null)(wrapped) // scalastyle:ignore null
    }
  }
