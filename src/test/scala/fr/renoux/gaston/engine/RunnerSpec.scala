...
class RunnerSpec extends AnyFlatSpec with Matchers with PrivateMethodTester {

  private implicit val context: Context = Context.Default

  private def run(problem: Problem, iterations: Long, globalSeed: Long): (Schedule, Long) = {
  ...
  "Given a fixed seed, the runner" should "always return the same result after 10 iterations" in {
    ...
    val otherResult = run(SimpleTestModel.Problems.Complete, iterations = 10, globalSeed = 43)
  ...
  "Given a fixed seed, the runner" should "always return the same result after 100 iterations" in {
    val results =
      for {_ <- 0 until 3} yield {
        run(SimpleTestModel.Problems.Complete, iterations = 100, globalSeed = 666)
      }
    ...
    val otherResult = run(SimpleTestModel.Problems.Complete, iterations = 100, globalSeed = 777)
