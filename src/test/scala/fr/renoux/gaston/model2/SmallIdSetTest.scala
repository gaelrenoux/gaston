...
class SmallIdSetTest extends TestBase {
  ...
  "added > on non-empty set > non-existing value" in {
  ...
  "added > on non-empty set > existing value" in {
    val set: SmallIdSet[TopicId] = SmallIdSet(testOkIds*)
    for (id <- testOkIds) {
      val s1 = set + id
      val s2 = set.added(id)
